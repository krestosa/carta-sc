...
class StaticAssetExternalizer {
  async run(): Promise<void> {
    this.#assertRepositoryBoundary();
    const files = pagesTextFiles();
    const rewriteStats = rewriteStaticReferences(files);
    this.#verifyRewriteStats(rewriteStats.replacements, rewriteStats.canonicalized);
    ...
    this.#verifyGeneratedAssets(
      generated.unexpected,
      generated.mirrored,
      mirrors.targets.size,
      files,
    );
  ...
  #verifyRewriteStats(replacements: number, canonicalized: number): void {
    assert(replacements > 0, 'No static asset references were externalized');
    assert(canonicalized > 0, 'Expected captured snapshot asset variants to be canonicalized');
  }

  #verifyGeneratedAssets(
    unexpected: readonly string[],
    mirrored: readonly string[],
    expectedMirroredCount: number,
    files: readonly string[],
  ): void {
    assert(
      unexpected.length === 0,
      `Unexpected static asset files remain in Pages artifact: ${unexpected.slice(0, 20).join(', ')}`,
    );
    assert(
      mirrored.length === expectedMirroredCount,
      'Mirrored SushiClub font count does not match downloaded font set',
    );
    verifyCriticalFonts(files, mirrored);
    verifyNoSnapshotReferences(files);
  }
}

export async function externalizeStaticAssets(): Promise<void> {
  await new StaticAssetExternalizer().run();
}
