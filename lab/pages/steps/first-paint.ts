...
export async function optimizeFirstPaint(): Promise<void> {
  ...
  html = deferInlineRuntime(html);
  html = injectCatalogToolsShell(html);

  const lazy = hardLazy(html);
  html = lazy.html;
  ...
  console.log(
    `First-viewport split: ${lazy.count} product images hard-lazy, same-origin mobile logo LCP, static catalog tools shell, ${split.criticalBytes} critical CSS bytes, ${split.deferredBytes} deferred CSS bytes, runtime gated behind logo paint, imgLiquid/Knormalize pruned.`,
  );
}
