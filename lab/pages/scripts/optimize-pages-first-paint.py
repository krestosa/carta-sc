...
def hard_lazy(html):
    ...
    def repl(m):
        ...
        if src:
            ...
            ia+=f' data-sc-src="{val}" loading="lazy" decoding="async" fetchpriority="low"'
...
def mirror_critical_media(html):
    ...
    def fetch_asset(url,name):
        ...
        raise SystemExit(f'critical media mirror failed for {url}: {error}')

    logo_re=re.compile(r'<img\b(?P<a>[^>]*\bsrc=["\'](?P<u>[^"\']*web-sushiclub2_black_m2\.png)["\'][^>]*)>',re.I)
    match=logo_re.search(html)
    if not match:raise SystemExit('mobile SushiClub logo missing before mirror')
    attrs=match.group('a');local=fetch_asset(match.group('u'),'mobile-logo');asset=local+'?v='+SHA
    style=re.search(r'\s+style=["\'](?P<s>[^"\']*)["\']',attrs,re.I)
    if style:
        kept=[]
        for declaration in style.group('s').split(';'):
            declaration=declaration.strip()
            if not declaration:continue
            name=declaration.split(':',1)[0].strip().lower() if ':' in declaration else ''
            if name in {'margin-left','transform'}:continue
            kept.append(declaration)
        replacement=(' style="'+'; '.join(kept)+'"') if kept else ''
        attrs=attrs[:style.start()]+replacement+attrs[style.end():]
    attrs=re.sub(r'\s+(?:width|height|loading|decoding|fetchpriority|data-sc-lcp-logo)=["\'][^"\']*["\']','',attrs,flags=re.I)
    attrs=re.sub(r'\bsrc=["\'][^"\']+["\']','src="'+asset+'"',attrs,count=1,flags=re.I)
    attrs+=' width="333" height="100" loading="eager" decoding="async" fetchpriority="high" data-sc-lcp-logo="1"'
    html=html[:match.start()]+'<img'+attrs+'>'+html[match.end():]
...
def verify(html,n,c,d):
    ...
    if re.search(r'data-sc-lcp-product=',html,re.I):raise SystemExit('stale product LCP promotion remains')
    ...
    logo=re.search(r'<img\b(?=[^>]*data-sc-lcp-logo=["\']1["\'])(?=[^>]*src=["\']_critical-media/mobile-logo\.png\?v='+re.escape(SHA)+r'["\'])[^>]*>',html,re.I)
    if not logo:raise SystemExit('same-origin mobile LCP logo missing')
    if not re.search(r'loading=["\']eager["\']',logo.group(0),re.I) or not re.search(r'fetchpriority=["\']high["\']',logo.group(0),re.I):raise SystemExit('mobile LCP logo lost eager/high priority')
    if re.search(r'\b(?:margin-left|transform)\s*:',logo.group(0),re.I):raise SystemExit('late-mutating mobile logo geometry remains')
    if not re.search(r'<link\b(?=[^>]*rel=["\']preload["\'])(?=[^>]*href=["\']_critical-media/mobile-logo\.png\?v='+re.escape(SHA)+r'["\'])(?=[^>]*fetchpriority=["\']high["\'])[^>]*>',html,re.I):raise SystemExit('same-origin mobile logo preload missing')
    if not (SITE/'_critical-media/mobile-logo.png').is_file():raise SystemExit('mirrored mobile logo file missing')
