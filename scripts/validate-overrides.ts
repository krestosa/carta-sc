...
function requirePaths(paths: readonly string[]): void {
  for (const required of paths) {
    validation.check(fs.existsSync(required), `Missing required frontend path: ${relativeTo(ROOT, required)}`);
  }
}
...
function validateSourceBoundaries(typeScriptFiles: readonly string[], sourceJavaScript: readonly string[]): void {
  if (sourceJavaScript.length > 0) {
    validation.fail(`Project-owned JS remains in override/: ${sourceJavaScript.map(relativeOverride).join(', ')}`);
  }

  const source = typeScriptFiles.map(readText).join('\n');
  validation.check(!/\bscrollRestoration\b/.test(source), 'Override must leave browser scroll restoration native');
  validation.check(!/\._data\s*\(/.test(source), 'Override must not depend on private jQuery _data internals');
  validation.check(!/\bdocument\.currentScript\b/.test(source), 'Override modules must not depend on document.currentScript');
  validation.check(
...
function validateCssManifest(): void {
  ...
  for (const reference of references) {
    ...
    validation.check(query === 'v=unversioned', `CSS import must use ?v=unversioned: ${reference}`);
    validation.check(fs.existsSync(path.resolve(OVERRIDE_DIR, assetPath)), `Missing CSS import target: ${assetPath}`);
  }
}
...
if (validation.errors.length === 0) {
  ...
  const sourceJavaScript = files.filter((file) => file.endsWith('.js'));
  const generatedJavaScript = walkFiles(GENERATED_DIR).filter((file) => file.endsWith('.js'));

  validateGeneratedJavaScript(generatedJavaScript);
  validateSourceBoundaries(typeScriptFiles, sourceJavaScript);
  ...
  if (validation.errors.length === 0) {
    console.log(
      `Override validation checked ${typeScriptFiles.length} TypeScript files, ${htmlFiles.length} templates, ${cssFiles.length} CSS files and ${generatedJavaScript.length} generated JS files.`,
    );
  }
}

validation.finish('Override validation failed', 'Override validation passed.');
