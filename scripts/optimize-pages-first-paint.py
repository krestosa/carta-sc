...
def split_css(html):
    ...
    if rm: critical.append(rm.group('c'))
    ...
    if rm:
        html=rob.sub('',html,count=1)
        html=re.sub(r'<link\b(?=[^>]*rel=["\']preconnect["\'])(?=[^>]*href=["\']https://fonts\.googleapis\.com/?["\'])[^>]*>\s*','',html,flags=re.I)
...
def verify(html,n,c,d):
    ...
    promoted=re.findall(r'<img\b[^>]*\bdata-sc-lcp-product=["\'][12]["\'][^>]*>',html,re.I)
    if len(promoted)!=2:raise SystemExit(f'mobile LCP product row is not promoted: {len(promoted)}')
    ...
    critical_style=re.search(r'<style id="sc-pages-critical-css">(?P<c>[\s\S]*?)</style>',html,re.I)
    if not critical_style or not re.search(r'@font-face\s*\{[^}]*font-family\s*:\s*["\']Roboto["\']',critical_style.group('c'),re.I):raise SystemExit('Roboto font faces are not critical')
    if re.search(r'@font-face\s*\{[^}]*font-family\s*:\s*["\']Roboto["\']',DEFERRED.read_text(encoding='utf-8',errors='ignore'),re.I):raise SystemExit('Roboto font faces remain deferred')
