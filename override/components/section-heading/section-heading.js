...
if(!SC||!C||!SC.motion||typeof SC.motion.whenReady!=='function'||SC.__sectionHeadingBooted)return;
SC.__sectionHeadingBooted=true;
var initialized=false,pending=false,splitPromise=null,motionDeps=null,generation=0,firstRaf=0,secondRaf=0,elements=[],splits=[],tweens=[],SPLIT_TEXT_ID='sc-gsap-splittext',RULE_PROPERTY='--sc-section-rule-scale',CFG={ruleDuration:.72,textDuration:.52,textStagger:.045,triggerStart:'clamp(top 90%)',lineOffsetPercent:105},REFRESH_DELAY=60;
...
function initMotion(deps,SplitText,token){
  if(initialized||token!==generation||!SplitText)return;initialized=true;pending=false;var gsap=deps.gsap;elements=targets();splits=[];tweens=[];
...
function init(){
  if(initialized||pending||!motionDeps||SC.motion.reduced())return;pending=true;var token=++generation;
  loadSplitText().then(function(SplitText){
    if(!SplitText||token!==generation||!pending||SC.motion.reduced()){if(token===generation)pending=false;return;}
    firstRaf=requestAnimationFrame(function(){firstRaf=0;if(token!==generation||!pending)return;secondRaf=requestAnimationFrame(function(){secondRaf=0;initMotion(motionDeps,SplitText,token);});});
  });
}
function destroy(){
