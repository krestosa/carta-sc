(function(){
'use strict';
var SC=window.SCOverride,C=SC&&SC.config,S=C&&C.selectors,K=C&&C.classes,I=C&&C.ids,P=C&&C.cssProperties,M=C&&C.motion,CFG=C&&C.sectionHeading;
...
function loadSplitText(){
  ...
  splitPromise=new Promise(function(resolve){
    var old=document.getElementById(I.gsapSplitText);if(old){if(old.dataset.loaded==='true'||window.SplitText)return resolve(window.SplitText||null);old.addEventListener('load',function(){resolve(window.SplitText||null);},{once:true});old.addEventListener('error',function(){resolve(null);},{once:true});return;}
    var script=document.createElement('script');script.id=I.gsapSplitText;script.src=C.urls.splitText;script.async=true;script.onload=function(){script.dataset.loaded='true';resolve(window.SplitText||null);};script.onerror=function(){resolve(null);};document.head.appendChild(script);
  });return splitPromise;
}
...
function initMotion(deps,SplitText){
  ...
  elements.forEach(function(el){
    ...
    if(parentCategory){el.classList.add(K.sectionRuleHost);if(initial)gsap.set(el,{[P.sectionRuleScale]:1});else{gsap.set(el,{[P.sectionRuleScale]:0});tweens.push(gsap.to(el,{[P.sectionRuleScale]:1,duration:CFG.ruleDuration,ease:M.easings.strongOut,overwrite:'auto',scrollTrigger:{trigger:el,start:CFG.triggerStart,once:true}}));}}
    if(initial)return;
    splits.push(SplitText.create(el,{type:'lines',mask:'lines',linesClass:K.sectionTextLine,autoSplit:true,onSplit:function(self){return gsap.from(self.lines,{yPercent:CFG.lineOffsetPercent,autoAlpha:0,duration:CFG.textDuration,ease:M.easings.strongOut,stagger:CFG.textStagger,overwrite:'auto',scrollTrigger:{trigger:el,start:CFG.triggerStart,once:true,invalidateOnRefresh:true}});}}));
  });
  ...
  function cleanup(){tweens.forEach(function(tween){if(tween&&tween.scrollTrigger)tween.scrollTrigger.kill();if(tween)tween.kill();});splits.forEach(function(split){if(split&&split.revert)split.revert();});elements.forEach(function(el){el.classList.remove(K.sectionRuleHost);gsap.set(el,{clearProps:P.sectionRuleScale});});}
