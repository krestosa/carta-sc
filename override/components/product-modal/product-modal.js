...
var ready=U.ready,activeModal=null,activeLink=null,closingModal=null,previousFocus=null,releaseBackground=null,closingRelease=null,closingRestore=null,openRaf=0,closeTimer=0,initialized=false,returnUrl='',PRODUCT_MODAL_CLOSE_DELAY=190,HASH_PREFIX='#producto-';
function noop(){}
...
function finishClose(modal,restoreFocus){if(closingModal!==modal)return;cancelCloseTimer();if(modal.parentNode)modal.parentNode.removeChild(modal);document.body.classList.remove('sc-product-modal-open');var release=closingRelease||noop,restore=closingRestore;closingModal=null;closingRelease=null;closingRestore=null;release();if(restoreFocus!==false)focus(restore);}
function closeModal(event,keepHash){if(event)event.preventDefault();if(!activeModal||closingModal)return;cancelOpenRaf();var modal=activeModal;closingRestore=previousFocus;closingRelease=releaseBackground||noop;activeModal=null;activeLink=null;closingModal=modal;previousFocus=null;releaseBackground=null;if(!keepHash)clearProductHash();M.close(modal);var delay=C.queries.reducedMotion.matches?0:PRODUCT_MODAL_CLOSE_DELAY;closeTimer=window.setTimeout(function(){closeTimer=0;finishClose(modal,true);},delay);}
function openModal(link,options){options=options||{};if(activeModal||closingModal||!link)return;var modal=buildModal(link);if(!modal)return;previousFocus=options.fromHash?null:link;returnUrl=options.fromHash?(location.pathname||'/')+(location.search||''):(location.pathname||'/')+(location.search||'')+(currentProductHash()?'':location.hash||'');document.body.appendChild(modal);document.body.classList.add('sc-product-modal-open');activeModal=modal;activeLink=link;releaseBackground=A.lockBackground(modal)||noop;if(!options.fromHash)writeProductHash(link);focus(modal.querySelector('.sc-product-modal__close'));cancelOpenRaf();openRaf=requestAnimationFrame(function(){openRaf=0;if(activeModal===modal){modal.classList.add('is-visible');M.open(modal);}});}
...
function destroy(){if(initialized){initialized=false;removeListeners();}cancelOpenRaf();cancelCloseTimer();var modal=activeModal||closingModal,restore=previousFocus||closingRestore,release=releaseBackground||closingRelease||noop;activeModal=null;activeLink=null;closingModal=null;previousFocus=null;releaseBackground=null;closingRelease=null;closingRestore=null;if(modal&&modal.parentNode)modal.parentNode.removeChild(modal);if(document.body)document.body.classList.remove('sc-product-modal-open');release();focus(restore);}
