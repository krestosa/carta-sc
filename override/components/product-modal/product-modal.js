...
function onMouseDown(event){if(!activeModal)return;var backdrop=event.target===activeModal||(event.target.classList&&event.target.classList.contains('sc-product-modal__scrim'));if(!backdrop)return;event.preventDefault();event.stopPropagation();if(event.stopImmediatePropagation)event.stopImmediatePropagation();closeModal();}
