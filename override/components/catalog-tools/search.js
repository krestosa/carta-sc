...
var C=SC.catalogTools=SC.catalogTools||{},inventory=[],searchable=[],groups=[],hosts=[],visibleItems=[],queryCandidates=[],lastQuery='',lastState='',active=false,installedRoot=null,installedInput=null,clearNode=null,statusNode=null,resultsNode=null,emptyNode=null,onInput=null,onKeyDown=null,onRootClick=null,onRootKeyDown=null,saveTimer=0,pendingSave='',epoch=0,captured=false,activeFilters=new Set();
...
function clearSearch(root,focusInput){if(!installedInput)return;installedInput.value='';queueSearchSave('');lastState='';apply(root,'');syncClear();if(focusInput)installedInput.focus();}
...
function destroy(){
  ...
  installedRoot=null;installedInput=null;clearNode=null;statusNode=null;resultsNode=null;emptyNode=null;onInput=null;onKeyDown=null;onRootClick=null;onRootKeyDown=null;inventory=[];searchable=[];groups=[];hosts=[];visibleItems=[];queryCandidates=[];lastQuery='';lastState='';pendingSave='';captured=false;activeFilters.clear();
}
function install(root){
  destroy();var input=root&&root.querySelector('.sc-catalog-search-input');if(!input)return function(){};installedRoot=root;installedInput=input;clearNode=root.querySelector('.sc-catalog-search-clear');statusNode=root.querySelector('.sc-catalog-search-status');resultsNode=root.querySelector('.sc-catalog-search-results');emptyNode=root.querySelector('.sc-catalog-search-empty-message');U.each(root.querySelectorAll('.sc-sort-control'),function(node){if(node.parentNode)node.parentNode.removeChild(node);});
  onInput=function(){queueSearchSave(input.value);lastState='';apply(root,input.value);syncClear();};
  onKeyDown=function(event){if(event.key!=='Escape'||!input.value)return;event.preventDefault();clearSearch(root,true);};
  onRootClick=function(event){var clear=event.target&&event.target.closest&&event.target.closest('.sc-catalog-search-clear');if(clear&&root.contains(clear)){event.preventDefault();event.stopPropagation();clearSearch(root,true);return;}if(handleFilter(root,event.target))event.preventDefault();};
  ...
  syncClear();requestAnimationFrame(function(){if(installedRoot!==root)return;ensureFilters(root);var saved=loadSearch();if(saved){input.value=saved;lastState='';apply(root,saved);}syncClear();});return function(){if(installedRoot===root)destroy();};
}
