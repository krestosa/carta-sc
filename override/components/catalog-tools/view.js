...
function animateMorph(host,from,to){
  ...
  clearViewIconMotion(host);
  var gsap=motionDeps.gsap,live=livePaths(host),targets=targetPaths(host,to),short=reducedMotion(),duration=short?.18:.48,ease=short?'power1.inOut':'power2.inOut',state={timeline:null};
  ...
  live.forEach(function(path,index){
    state.timeline.to(path,{duration:duration,ease:ease,morphSVG:{shape:targets[index]},overwrite:'auto'},short?0:Math.floor(index/2)*.025);
  });
}
