...
function stopMenu(root,preserve){var menu=root&&root.querySelector('.sc-theme-menu'),g=gsap(),items=options(root),active=!!menuTimeline,snapshot=null;if(active&&preserve&&menu&&g){snapshot={height:menu.getBoundingClientRect().height,opacity:parseFloat(getComputedStyle(menu).opacity),items:items.map(function(item){var value=parseFloat(getComputedStyle(item).opacity);return isFinite(value)?value:1;})};if(!isFinite(snapshot.opacity))snapshot.opacity=1;}if(menuTimeline){menuTimeline.kill();menuTimeline=null;}if(menu&&g&&(!preserve||!active)){g.set([menu].concat(items),{clearProps:'height,opacity,overflow,visibility,pointerEvents,transform,willChange'});}return snapshot;}
...
function setOpen(root,on,focus){
  ...
  if(on){
    ...
    g.set(menu,{height:startHeight,opacity:startOpacity,overflow:'hidden',visibility:'visible',pointerEvents:'auto',willChange:'height,opacity'});items.forEach(function(item,index){g.set(item,{opacity:carry&&isFinite(carry.items[index])?carry.items[index]:0,willChange:'opacity'});});
