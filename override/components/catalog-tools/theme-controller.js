...
function setOpen(root,on,focus){
  ...
  if(on){
    ...
    var full=Math.max(1,menu.scrollHeight||menu.offsetHeight),duration=.50,itemDuration=.25,step=(duration-itemDuration)/count;
  ...
  var current=Math.max(1,menu.getBoundingClientRect().height||menu.scrollHeight||1),target=current*.35,closeDuration=.15,fadeDuration=.05,fadeStart=.10,itemDurationClose=.05,itemInitial=.05,itemStep=(closeDuration-itemInitial-itemDurationClose)/count,closingItems=items.slice().reverse();
