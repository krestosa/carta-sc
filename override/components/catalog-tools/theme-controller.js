...
function animateGeometry(root,from,to){
  ...
  stopMain();applyStatic(root,from);
  ...
  mainTimeline.to(p.button,{rotation:kick,duration:mobile?.12:.15,ease:'power2.out'},0)
    .to(p.button,{rotation:0,duration:mobile?.17:.21,ease:'power3.out'},mobile?.095:.12);
  if(from==='light'){
