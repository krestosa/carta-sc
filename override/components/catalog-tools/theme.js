...
var MODES=['system','light','dark'],STORE_KEY='scTheme:v1',doc=document.documentElement,systemDark=window.matchMedia('(prefers-color-scheme: dark)'),finePointer=window.matchMedia('(hover:hover) and (pointer:fine)'),persistHandle=0,persistMode='',switchToken=0,switchTimers=[],iconToken=0,iconTimeline=null,motionDeps=null,SWITCH_COMMIT=210,SWITCH_IN=225,SWITCH_END=545;
...
function commit(root,mode,persist,skipSync){
  ...
  setResolved(actual);if(!skipSync)sync(root,mode);if(visualChanged)emit(mode,actual);if(persist&&modeChanged)scheduleSave(mode);
}

function iconLayers(root){
  var host=root&&root.querySelector('[data-sc-theme-icon]');if(!host)return null;
  var system=host.querySelector('[data-sc-theme-layer="system"]'),light=host.querySelector('[data-sc-theme-layer="light"]'),dark=host.querySelector('[data-sc-theme-layer="dark"]');
  return system&&light&&dark?{system:system,light:light,dark:dark,all:[system,light,dark]}:null;
}
function clearIconStyles(gsap,layers){if(gsap&&layers)gsap.set(layers.all,{clearProps:'transform,opacity,visibility'});}
function cancelIconMotion(root,clearInline){
  iconToken++;if(iconTimeline){try{iconTimeline.kill();}catch(_){}iconTimeline=null;}
  if(root){root.removeAttribute('data-sc-theme-animating');if(clearInline&&motionDeps&&motionDeps.gsap)clearIconStyles(motionDeps.gsap,iconLayers(root));}
  return iconToken;
}
function finalizeIcon(root,token){if(token!==iconToken||!root)return;var gsap=motionDeps&&motionDeps.gsap;if(iconTimeline){try{iconTimeline.kill();}catch(_){}iconTimeline=null;}if(gsap)clearIconStyles(gsap,iconLayers(root));root.removeAttribute('data-sc-theme-animating');}
function animateModeChange(root,from,to){
  if(!root)return;from=normalize(from)||'system';to=normalize(to)||'system';syncMeta(root,to);
  var layers=iconLayers(root),gsap=motionDeps&&motionDeps.gsap;
  if(!layers||from===to||reducedMotion()||!gsap){sync(root,to);return;}
  var outgoing=layers[from],incoming=layers[to],direction=MODES.indexOf(to)>=MODES.indexOf(from)?1:-1,token=cancelIconMotion(root,true);
  root.setAttribute('data-sc-theme-animating','true');root.setAttribute('data-sc-theme-mode',to);
  gsap.set(layers.all,{autoAlpha:0,scale:.84,rotation:0,x:0,y:0,transformOrigin:'50% 50%',force3D:false});
  gsap.set(outgoing,{autoAlpha:1,scale:1,rotation:0,x:0,y:0,transformOrigin:'50% 50%',force3D:false});
  gsap.set(incoming,{autoAlpha:0,scale:.84,rotation:8*direction,x:1.5*direction,y:0,transformOrigin:'50% 50%',force3D:false});
  iconTimeline=gsap.timeline({onComplete:function(){finalizeIcon(root,token);}})
    .to(outgoing,{autoAlpha:0,scale:.84,rotation:-8*direction,x:-1.5*direction,duration:.2,ease:'power2.in',transformOrigin:'50% 50%',force3D:false},0)
    .to(incoming,{autoAlpha:1,scale:1,rotation:0,x:0,duration:.34,ease:'power3.out',transformOrigin:'50% 50%',force3D:false},.07);
}
function apply(root,mode,persist){cancelIconMotion(root,true);commit(root,mode,persist,false);}
...
function transitionApply(root,mode,persist){
  mode=normalize(mode)||'system';
  var current=selected(),visual=normalize(root&&root.getAttribute('data-sc-theme-mode')||'')||current,actual=resolved(mode),currentActual=doc.getAttribute('data-sc-theme-resolved')||resolved(current);
  ...
  clearSwitch();
  var token=switchToken,surface='';try{surface=getComputedStyle(doc).getPropertyValue('--sc-color-surface').trim();}catch(_){}if(surface)doc.style.setProperty('--sc-theme-transition-surface',surface);
  doc.classList.add('sc-theme-transitioning');doc.setAttribute('data-sc-theme-transition-phase','idle');
  requestAnimationFrame(function(){if(token===switchToken)doc.setAttribute('data-sc-theme-transition-phase','out');});
  switchTimers.push(setTimeout(function(){if(token===switchToken)commit(root,mode,persist,true);},SWITCH_COMMIT));
  switchTimers.push(setTimeout(function(){if(token===switchToken)doc.setAttribute('data-sc-theme-transition-phase','in');},SWITCH_IN));
  switchTimers.push(setTimeout(function(){if(token!==switchToken)return;switchTimers.length=0;doc.classList.remove('sc-theme-transitioning');doc.removeAttribute('data-sc-theme-transition-phase');doc.style.removeProperty('--sc-theme-transition-surface');},SWITCH_END));
}
...
function install(root){
  ...
  apply(root,load(),false);var closeTimer=0,hovering=false;
...
if(SC.motion&&typeof SC.motion.whenLoaded==='function')SC.motion.whenLoaded(function(deps){motionDeps=deps;});
else if(SC.motion&&typeof SC.motion.whenReady==='function')SC.motion.whenReady(function(deps){motionDeps=deps;});
