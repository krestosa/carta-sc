...
var MODES=['system','light','dark'],STORE_KEY='scTheme:v1',doc=document.documentElement,systemDark=window.matchMedia('(prefers-color-scheme: dark)');
var ICONS={
...
function syncIcon(root,mode){
  ...
  if(!path)return;
  if(previous&&previous!==mode&&SC.motion&&SC.motion.morphIcon)SC.motion.morphIcon(path,ICONS[mode],{duration:.3});else path.setAttribute('d',ICONS[mode]);
...
function apply(root,mode,persist){
  ...
  if(persist)save(mode);syncIcon(root,mode);emit(mode);
}
