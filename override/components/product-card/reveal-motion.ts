...
const MOTION = {
  columnDelay: motionConfig.durations.short2,
  columnDelayMax: motionConfig.durations.medium1,
  revealDuration: motionConfig.durations.medium4,
} as const;
...
class ProductCardRevealController {
  ...
  revealVisibleCards = (): void => {
    ...
    for (const card of this.#cards) {
      ...
      if (rect.bottom <= 0) this.#finish(card);
