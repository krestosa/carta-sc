...
var ready=U.ready,S=C.selectors,desktopQuery=C.queries.desktop,resizeObserver=null,cardObserver=null,cardRaf=0,lastWidth=-1,resizeFallback=false,initialized=false,initialQueue=[],initialIdle=0,initialTimer=0,initialDone=false,RESIZE_WIDTH_TOLERANCE=.5,INITIAL_BATCH=6,INITIAL_BUDGET_MS=8;
...
function installResizeTracking(){var root=document.querySelector(S.container)||document.body;if(!root||resizeObserver||resizeFallback)return;if(!window.ResizeObserver){window.addEventListener('resize',P.scheduleDescriptionMeasure,{passive:true});resizeFallback=true;return;}lastWidth=-1;resizeObserver=new ResizeObserver(function(entries){var entry=entries[0],width=entry&&entry.contentRect?entry.contentRect.width:root.clientWidth;if(lastWidth>=0&&Math.abs(width-lastWidth)<RESIZE_WIDTH_TOLERANCE)return;lastWidth=width;P.scheduleDescriptionMeasure();});resizeObserver.observe(root);}
...
function scheduleInitialBatch(){if(!initialized||initialIdle||initialTimer)return;if(typeof window.requestIdleCallback==='function'){initialIdle=window.requestIdleCallback(runInitialBatch,{timeout:500});return;}initialTimer=window.setTimeout(function(){runInitialBatch(null);},16);}
