...
parts.setupReveal=function(gsap,ST,profile,reduce){
  ...
  function revealInitial(batch){
    ...
    /* Preserve the old two-frame paint separation: the hidden state must reach
       the compositor before visibility/opacity animate, otherwise first-row
       cards appear instantly. The CSS visibility selector supplies the small
       upward translate without touching transform/imgLiquid geometry. */
    initialRafA=requestAnimationFrame(function(){
      initialRafA=0;
      initialRafB=requestAnimationFrame(function(){
        initialRafB=0;
        if(suppressReveal()){showNow(batch);return;}
        gsap.to(batch,{autoAlpha:1,duration:CFG.revealDuration,stagger:CFG.initialStagger,ease:M.easings.out,overwrite:'auto',onComplete:function(){gsap.set(batch,{clearProps:'opacity,visibility'});}});
      });
    });
  }
  ...
  if(staticCards.length)showNow(staticCards);
  if(behind.length)showNow(behind);
  if(initial.length)revealInitial(initial);
  if(deferred.length){
  ...
  return function(){
    ...
    if(initialRafA)cancelAnimationFrame(initialRafA);
    if(initialRafB)cancelAnimationFrame(initialRafB);
