...
parts.setupReveal=function(gsap,ST,profile,reduce){
  ...
  cards.forEach(arm);
  var container=document.querySelector(S.container);if(container&&window.MutationObserver){mutationObserver=new MutationObserver(function(mutations){mutations.forEach(function(mutation){if(mutation.type!=='attributes'||mutation.attributeName!=='hidden')return;var target=mutation.target;if(target.matches&&target.matches(S.productCard)&&!target.hidden)arm(target);});});mutationObserver.observe(container,{subtree:true,attributes:true,attributeFilter:['hidden']});}
