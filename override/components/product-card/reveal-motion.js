/* Reveal de tarjetas con opacidad, desplazamiento corto y stagger. Conserva rescates
   para layouts tardíos y evita que ScrollTrigger e IntersectionObserver dupliquen entradas. */
(function(){
'use strict';
var SC=window.SCOverride,C=SC&&SC.config,S=C&&C.selectors,M=C&&C.motion,CFG={behindViewportOffset:-20,initialThreshold:.96,initialDuration:.34,initialStagger:.032,initialLeadDelay:.30,revealDuration:.34,revealStagger:.03,batchInterval:.06,reflowDuration:.22,rescueBottomOffset:-30,rescueOpacityThreshold:.05,rescueDelay:900};if(!SC||!C||SC.__productCardRevealMotionBooted)return;SC.__productCardRevealMotionBooted=true;
var parts=SC.productCardMotionParts=SC.productCardMotionParts||{};
parts.setupReveal=function(gsap,ST,profile,reduce){
  var cards=gsap.utils.toArray(S.productCards),triggers=[],timer=0,observer=null,pending=null,initialRafA=0,initialRafB=0,revealAhead=Math.max(0,Number(profile&&profile.revealAhead)||0);
  function noop(){}
  ...
  function clear(batch){gsap.set(batch,{clearProps:'transform,opacity,visibility,willChange'});}
  ...
  function showNow(batch){batch=takePending(batch);if(!batch.length)return;gsap.killTweensOf(batch);gsap.set(batch,{autoAlpha:1,y:0,force3D:false});clear(batch);}
  function reveal(batch){
    batch=takePending(batch);if(!batch.length)return;gsap.killTweensOf(batch);
    gsap.to(batch,{autoAlpha:1,y:0,duration:CFG.revealDuration,stagger:CFG.revealStagger,ease:M.easings.out,force3D:false,overwrite:'auto',onComplete:function(){clear(batch);}});
  }
  function revealViewport(){
    ...
    if(!batch.length)return;if(reduce||suppressReveal()){showNow(batch);return;}batch=takePending(batch);if(!batch.length)return;gsap.killTweensOf(batch);gsap.to(batch,{autoAlpha:1,y:0,duration:CFG.reflowDuration,ease:M.easings.out,force3D:false,overwrite:true,onComplete:function(){clear(batch);}});
  }
  ...
  /* Recupera el reveal histórico: estado inicial pintado y cards entrando desde abajo. */
  if(initial.length){
    gsap.set(initial,{autoAlpha:0,y:profile.initialY,force3D:false,willChange:'transform,opacity'});
    initialRafA=requestAnimationFrame(function(){initialRafA=0;initialRafB=requestAnimationFrame(function(){initialRafB=0;gsap.to(initial,{autoAlpha:1,y:0,duration:CFG.initialDuration,delay:CFG.initialLeadDelay,stagger:CFG.initialStagger,ease:M.easings.out,force3D:false,overwrite:'auto',onComplete:function(){clear(initial);}});});});
  }
  if(deferred.length){
    pending=new Set(deferred);gsap.set(deferred,{autoAlpha:0,y:profile.revealY,force3D:false,willChange:'transform,opacity'});
  ...
  return function(){
    if(parts.revealViewport===revealViewport)parts.revealViewport=null;if(initialRafA)cancelAnimationFrame(initialRafA);if(initialRafB)cancelAnimationFrame(initialRafB);if(timer)clearTimeout(timer);if(observer)observer.disconnect();if(pending)pending.clear();triggers.forEach(function(trigger){if(trigger&&trigger.kill)trigger.kill();});gsap.killTweensOf(cards);clear(cards);
  };
};
})();
