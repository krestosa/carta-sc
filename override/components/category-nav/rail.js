...
var railRaf=0,overflowDirty=true,centerPending=false,mobileInitialized=false;
function railState(){
  ...
  if(rail){if(N.mq.matches){centerPending=false;if(overflowDirty)C.hide(rail);}else{if(!mobileInitialized&&mobileScroller){mobileScroller.scrollLeft=0;mobileInitialized=true;}if(overflowDirty)C.overflow(rail,mobileScroller);if(centerPending){P.centerActive(mobileScroller);centerPending=false;}}}
