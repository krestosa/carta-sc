(function(){
'use strict';
var SC=window.SCOverride,C=SC&&SC.config,K=C&&C.classes,N=SC&&SC.categoryNav,A=N&&N.categoryActive,I=N&&N.categoryIndicator;
if(!SC||!C||!N||!A||!I||SC.__categoryNavScrollSpyBooted)return;SC.__categoryNavScrollSpyBooted=true;
var metrics=[],spyRaf=0,heldTarget=null,heldUntil=0,SPY_HOLD_MS=1800;
...
function current(){if(!metrics.length)return null;var mark=(pageYOffset||document.documentElement.scrollTop||0)+N.offset()+N.currentMarkOffset,target=metrics[0].target;for(var i=0;i<metrics.length;i++){if(metrics[i].top<=mark)target=metrics[i].target;else break;}return target;}
