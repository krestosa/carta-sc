...
function hold(target){heldTarget=target||null;heldUntil=heldTarget?performance.now()+1800:0;}
function release(){heldTarget=null;heldUntil=0;}
function spy(){
  spyRaf=0;var target=current(),active=A.current();
  if(heldTarget){
    if(target===heldTarget||performance.now()>=heldUntil)release();
    else{if(active&&I.isDirty())I.move(active,false);return;}
  }
