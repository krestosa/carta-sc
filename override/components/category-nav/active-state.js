(function(){
'use strict';
var SC=window.SCOverride,U=SC&&SC.utils,N=SC&&SC.categoryNav,I=N&&N.categoryIndicator;
...
function setActive(target,animate){
  ...
  each(document.querySelectorAll('.JSgoMenu'),function(select){for(var i=0;i<select.options.length;i++)if(N.anchor(select.options[i].value)===target){if(select.value!==select.options[i].value)select.value=select.options[i].value;break;}});
  I.move(target,animate&&changed);
  if(changed&&previous&&N.requestCenterActive)N.requestCenterActive();else if(N.scheduleRail)N.scheduleRail();
}
