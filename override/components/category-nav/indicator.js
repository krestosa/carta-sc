...
var each=U.each,entries=[],dirty=true,raf=0,lastFrame=0;
...
function bindScroll(item){
  ...
  item.onScroll=function(){
    var t=now(),x=el.scrollLeft||0,dt=(t-item.scrollT)/1000;
    if(dt>.003&&dt<.16){var v=-(x-item.scrollX)/dt;item.scrollV=item.scrollV*.32+v*.68;}
    item.scrollX=x;item.scrollT=t;item.scrollUntil=t+90;wake();
  };
...
function entry(root){
  ...
  for(var i=entries.length-1;i>=0;i--){
    if(entries[i].root!==root)continue;
    if(entries[i].host===host){item=entries[i];break;}
    destroy(entries[i]);entries.splice(i,1);
  }
  if(!item){
    var line=T.clone('category-indicator');host.classList.add('sc-category-motion-root');host.appendChild(line);if(host.classList.contains('topShopMenuMobileScroller'))line.style.setProperty('bottom','0','important');
    item={root:root,host:host,line:line,init:false,left:0,right:1,targetLeft:0,targetRight:1,vl:0,vr:0,warp:0,vw:0,scrollEl:null,onScroll:null,scrollX:0,scrollT:0,scrollV:0,scrollUntil:0};entries.push(item);
  }
...
function render(item,dt,t){
  ...
  if(reduced()){snap(item,item.targetLeft,item.targetRight);return false;}
  var s=spring(item.left,item.vl,item.targetLeft,EDGE_K,EDGE_D,dt);item.left=s[0];item.vl=s[1];
  s=spring(item.right,item.vr,item.targetRight,EDGE_K,EDGE_D,dt);item.right=s[0];item.vr=s[1];
  if(t>item.scrollUntil)item.scrollV*=Math.exp(-SCROLL_DECAY*dt);
  var width=Math.max(6,item.targetRight-item.targetLeft),visualVelocity=(item.vl+item.vr)*.5+item.scrollV;
  var maxWarp=Math.min(12,Math.max(5,width*.14)),warpTarget=Math.abs(visualVelocity)<32?0:clamp(visualVelocity*WARP_GAIN,-maxWarp,maxWarp);
  s=spring(item.warp,item.vw,warpTarget,WARP_K,WARP_D,dt);item.warp=s[0];item.vw=s[1];
  var m=Math.abs(item.warp),shift=item.warp*.22,left=item.left-m*.5+shift,right=item.right+m*.5+shift;
...
function frame(t){
  raf=0;var dt=clamp((t-lastFrame)/1000,.001,.032);lastFrame=t,active=false;
  for(var i=entries.length-1;i>=0;i--){var item=entries[i];if(!document.documentElement.contains(item.root)||!document.documentElement.contains(item.host)){destroy(item);entries.splice(i,1);continue;}bindScroll(item);if(render(item,dt,t))active=true;}
  if(active)wake();
}
function move(target,animate){
  each(document.querySelectorAll('.wrapp-nav-tabsTopShop .nav-tabsTopShop,.wtopShopMenuMobile .topShopMenuMobile .nav-tabs,.topShopMenuMobileScroller .nav-tabsTopShop'),function(root){
    ...
    var link=N.links(root).find(function(node){return N.anchor(node.getAttribute('href'))===target&&U.visible(node);});if(!link)return;
    ...
    item.targetLeft=x;item.targetRight=x+width;item.line.style.opacity='1';item.line.style.transformOrigin='0 50%';
    if(!item.init||!animate||reduced())snap(item,x,x+width);else wake();
  });dirty=false;
}
