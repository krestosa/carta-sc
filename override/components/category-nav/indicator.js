...
function clearTransform(item){if(!item||!item.line)return;var g=gsap();if(g)g.set(item.line,{clearProps:'transform'});else item.line.style.removeProperty('transform');}
...
function animate(item,next){
  ...
  item.tween=g.to(item.line,{x:0,scaleX:1,duration:spec.duration,ease:spec.ease,overwrite:'auto',force3D:true,onComplete:function(){item.tween=null;g.set(item.line,{clearProps:'transform'});item.line.style.removeProperty('will-change');}});
}
