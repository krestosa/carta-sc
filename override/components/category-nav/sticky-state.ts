...
function setStuck(host:HTMLElement|null,on:boolean):void{if(!host)return;var changed=host.classList.contains('sc-is-stuck')!==on;host.classList.toggle('sc-is-stuck',on);if(changed){if(N.invalidateOffset)N.invalidateOffset();if(N.refreshMetrics)N.refreshMetrics();}}
