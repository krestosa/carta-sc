...
/* Coordina la trayectoria programática y pausa temporalmente el scroll-spy. */
...
function scrollEase(progress){return SC.motion&&SC.motion.curve?SC.motion.curve('standard')(progress):(1-Math.pow(1-progress,4));}
...
function animateScroll(target,token,duration,destination){
  var startY=pageYOffset||document.documentElement.scrollTop||0,start=performance.now(),ms=duration*1000;
  function frame(now){if(token!==autoToken)return;if(offsetCache===null)destination=targetYFromCurrentOffset(target);var p=Math.min(1,(now-start)/ms);scrollTo(0,startY+(destination-startY)*scrollEase(p));if(p<1)autoRaf=requestAnimationFrame(frame);else{autoRaf=0;finishAutoScroll(token,target);}}
