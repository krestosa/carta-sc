...
function persistentSticky(node,style,rect){
  if(style.position!=='sticky')return 0;
  var isRail=(S.categoryToolbar&&node.matches(S.categoryToolbar))||node.matches(N.selectors.mobileWrapper);if(!isRail)return rect.top<=CFG.currentMarkOffset&&rect.bottom>0?rect.bottom:0;
  var top=parseFloat(style.top);if(!isFinite(top))top=0;return Math.max(0,top+rect.height);
}
function offset(){
  if(offsetCache!==null)return offsetCache;var bottom=0;
  [".topBar",".topShop",S.categoryToolbar,N.selectors.mobileWrapper].forEach(function(selector){if(!selector)return;each(document.querySelectorAll(selector),function(node){
    if(!U.visible(node))return;var style=getComputedStyle(node),rect=node.getBoundingClientRect(),candidate=0;
    if(style.position==='fixed'&&rect.top<=CFG.currentMarkOffset&&rect.bottom>0)candidate=rect.bottom;else candidate=persistentSticky(node,style,rect);
...
function targetYFromCurrentOffset(target){
  var y=target.getBoundingClientRect().top+(pageYOffset||document.documentElement.scrollTop||0)-offset(),max=Math.max(0,document.documentElement.scrollHeight-innerHeight);return Math.max(0,Math.min(max,y));
}
...
function animateScroll(target,token,duration){
  var startY=pageYOffset||document.documentElement.scrollTop||0,start=performance.now(),ms=duration*1000;
  function frame(now){
    if(token!==autoToken)return;var p=Math.min(1,(now-start)/ms),destination=targetYFromCurrentOffset(target);scrollTo(0,startY+(destination-startY)*ease(p));
    if(p<1)autoRaf=requestAnimationFrame(frame);else{autoRaf=0;finishAutoScroll(token,target);}
  }
  autoRaf=requestAnimationFrame(frame);
}
function scrollToTarget(target,plan){
  invalidateOffset();plan=plan||scrollPlan(target);cancelAutoScroll(false);var token=++autoToken;setProgrammatic(true,false);
  if(!plan.duration){scrollTo(0,targetY(target));finishAutoScroll(token,target);return;}animateScroll(target,token,plan.duration);
}
