...
function parentLink(a){return !!(a&&a.matches&&a.matches(S.categoryParentLink)&&!a.classList.contains('anchorLinkSub')&&!a.closest(S.categoryDropdown));}
...
function onCategory(event){if(event.defaultPrevented||event.button>0||event.metaKey||event.ctrlKey||event.shiftKey||event.altKey)return;var link=event.target&&event.target.closest?event.target.closest(S.categoryLink):null;if(!link||link.closest(S.categoryDropdown)||!(link.closest(S.categoryToolbar)||link.closest(S.categoryMobileWrapper+' '+S.categoryMobileRail)))return;var target=anchor(link.getAttribute('href'));if(!target)return;event.preventDefault();event.stopImmediatePropagation();closeLegacy();cleanHash();activateAndScroll(target);}
