(function(){
'use strict';
var SC=window.SCOverride,C=SC&&SC.config,S=C&&C.selectors,K=C&&C.classes,P=C&&C.prefixes,CFG=C&&C.categoryNav,N=SC&&SC.categoryNav,T=SC&&SC.templates;if(!SC||!N||!T||SC.__categoryNavRailControlsBooted)return;SC.__categoryNavRailControlsBooted=true;
function arrow(host,scroller,dir){var button=host.querySelector('.'+P.railArrowClass+dir);if(button)return button;button=T.clone(C.templates.names.categoryArrowPrefix+dir);button.addEventListener('click',function(){var x=Math.max(CFG.railStepMin,Math.round(scroller.clientWidth*CFG.railStepRatio))*(dir==='left'?-1:1);try{scroller.scrollBy({left:x,behavior:C.queries.reducedMotion.matches?'auto':'smooth'});}catch(_){scroller.scrollLeft+=x;}if(N.scheduleRail)N.scheduleRail();});host.appendChild(button);return button;}
