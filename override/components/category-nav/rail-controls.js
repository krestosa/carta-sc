(function(){
'use strict';
var SC=window.SCOverride,C=SC&&SC.config,S=C&&C.selectors,K=C&&C.classes,N=SC&&SC.categoryNav,T=SC&&SC.templates;if(!SC||!N||!T||SC.__categoryNavRailControlsBooted)return;SC.__categoryNavRailControlsBooted=true;var RAIL_STEP_MIN=140,RAIL_STEP_RATIO=.65;
function arrow(host,scroller,dir){var button=host.querySelector('.'+"sc-rail-arrow--"+dir);if(button)return button;button=T.clone('category-arrow-'+dir);button.addEventListener('click',function(){var x=Math.max(RAIL_STEP_MIN,Math.round(scroller.clientWidth*RAIL_STEP_RATIO))*(dir==='left'?-1:1);try{scroller.scrollBy({left:x,behavior:C.queries.reducedMotion.matches?'auto':'smooth'});}catch(_){scroller.scrollLeft+=x;}if(N.scheduleRail)N.scheduleRail();});host.appendChild(button);return button;}
