...
SC.motion.whenReady(function(deps){
  ...
  window.jQuery(document).off('shown.bs.dropdown.scUxMotion').on('shown.bs.dropdown.scUxMotion',function(event){
    ...
    gsap.fromTo(menu,{autoAlpha:0,y:reduce?0:OFFSET_Y},{
      autoAlpha:1,y:0,duration:reduce?REDUCED_DURATION:DURATION,ease:M.easings.out,overwrite:true,clearProps:'transform,opacity,visibility'
    });
  });
});
})();