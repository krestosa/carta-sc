...
function registerDocument(source,path){var doc=new DOMParser().parseFromString(source,'text/html');var templates=doc.querySelectorAll('template[data-sc-template]');if(!templates.length)throw new Error('[SushiClub templates] Sin templates en '+path);Array.prototype.forEach.call(templates,function(template){register((template.getAttribute('data-sc-template')||'').trim(),template.innerHTML);});}
