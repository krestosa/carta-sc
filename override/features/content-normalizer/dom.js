(function(){
'use strict';
var SC=window.SCOverride,CFG=SC&&SC.config,S=CFG&&CFG.selectors,LOCALE=CFG&&CFG.catalog&&CFG.catalog.locale,C=SC&&SC.contentNormalizer,R=C&&C.rules;
...
function textNodes(root,skipSelector){
  var nodes=[],walker=document.createTreeWalker(root,NodeFilter.SHOW_TEXT,{acceptNode:function(node){var parent=node.parentElement;if(!parent||parent.closest(S.nonContent)||(skipSelector&&parent.closest(skipSelector)))return NodeFilter.FILTER_REJECT;return NodeFilter.FILTER_ACCEPT;}}),node;
...
function unwrapTypography(description){
  description.querySelectorAll(S.contentBold).forEach(function(node){var parent=node.parentNode;if(!parent)return;while(node.firstChild)parent.insertBefore(node.firstChild,node);parent.removeChild(node);});
  description.querySelectorAll(S.inlineStyle).forEach(function(node){['font-family','font-size','font-weight','font-style','text-transform','letter-spacing','line-height'].forEach(function(prop){node.style.removeProperty(prop);});if(!node.getAttribute('style')||!node.getAttribute('style').trim())node.removeAttribute('style');});
}
