...
function capitalize(lower){if(PROTECTED[lower])return PROTECTED[lower];return lower.replace(/[a-záéíóúüñ]/i,function(letter){return letter.toLocaleUpperCase('es-AR');});}
function smartCase(text,state,removePeriods){
  ...
  while((match=rx.exec(text))){
    ...
    var raw=match[0],lower=raw.toLocaleLowerCase('es-AR'),numeric=/^\d/.test(raw),word;
