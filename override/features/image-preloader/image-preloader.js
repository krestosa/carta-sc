...
function bind(url,stage){if(!stage)return;if(settled.has(url)){markReady(stage);return;}markLoading(stage);var nodes=waiters.get(url);if(!nodes){nodes=new Set();waiters.set(url,nodes);}nodes.add(stage);}
...
function queue(url,priority,stage){url=absolute(url);if(!url){if(stage)markReady(stage);return;}bind(url,stage);if(settled.has(url)||seen.has(url))return;seen.add(url);(priority?high:low).push(url);pump();}
...
function nativeDone(img,stage){var url=imageUrl(img);if(url)settle(url);markReady(stage);}
function collectImage(img,stage){if(!img)return;stage=stage||stageFor(img);var priority=nearViewport(stage||img),url=imageUrl(img);if(priority){try{img.loading='eager';img.fetchPriority='high';}catch(_){}}if(stage){if(img.complete){if(url)settle(url);markReady(stage);}else{markLoading(stage);if(!img.__scImagePreloaderBound){img.__scImagePreloaderBound=true;img.addEventListener('load',function(){nativeDone(img,stage);});img.addEventListener('error',function(){nativeDone(img,stage);});}}}queue(url,priority,stage);}
...
function collectStage(stage){if(!stage)return;var img=stage.querySelector('img[src],img[srcset]');if(img){collectImage(img,stage);return;}var urls=backgroundUrls(stage),priority=nearViewport(stage);if(!urls.length){markReady(stage);return;}urls.forEach(function(url){queue(url,priority,stage);});}
