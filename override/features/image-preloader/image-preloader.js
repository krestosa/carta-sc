...
function collectImage(img,stage){if(!img)return;stage=stage||stageFor(img);var active=setPriority(img);if(!stage)return;bindNativeImage(img,stage,active);if(img.complete){markReady(stage);unbindNativeImage(img);}}
